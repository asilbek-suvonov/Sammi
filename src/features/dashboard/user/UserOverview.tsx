...
import { BookOpen, CheckCircle2, Clock, PlayCircle, TrendingUp } from 'lucide-react'
...
const UserOverview = () => {
  ...
  const enrolledDetails = enrolledCourses.map((ec) => {
    ...
    return { course, progress, watched, total: totalLessons, remaining, enrolledAt: ec.enrolledAt }
  }).filter(Boolean) as {
  ...
  const totalWatched = enrolledDetails.reduce((acc, d) => acc + d.watched, 0)
  const completedCourses = enrolledDetails.filter((d) => d.progress === 100).length
  const inProgress = enrolledDetails.filter((d) => d.progress > 0 && d.progress < 100).length
        ...
        {enrolledDetails.length === 0 ? (
          <div className='flex flex-col items-center justify-center rounded-xl border border-dashed py-16 text-center'>
            ...
            <p className='mt-1 text-xs text-muted-foreground'>
              Go to{' '}
              <Link to='/dashboard/courses' className='text-primary underline-offset-2 hover:underline'>
                Courses
              </Link>{' '}
              to start learning.
            </p>
              ...
              {enrolledDetails.map(({ course, progress, watched, total, remaining }) => (
                ...
                >
                  <div className='overflow-hidden rounded-xl border bg-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md'>
                    ...
                    <div className='p-3'>
                      <p className='mb-2 text-sm font-medium leading-tight line-clamp-2'>
                        {course.title}
                      </p>
