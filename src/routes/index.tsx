        ...
        <section id='courses' className='space-y-6'>
          ...
          <div className='grid gap-4 sm:grid-cols-2 md:grid-cols-3'>
            {courses.map((course) => (
              <Link
                to='/course/$id'
                params={{ id: course.id ?? course.title }}
                key={course.title}
              >
                <Card
                  key={course.title}
                  className='group cursor-pointer gap-0 overflow-hidden p-3 transition-shadow duration-200 hover:shadow-md dark:bg-neutral-800/40'
                >
                  <div className='relative overflow-hidden'>
                    ...
                    <div className='absolute top-2.5 left-2.5'>
                      <Badge
                        ...
                        className='rounded-md bg-accent text-[11px] text-black dark:text-white'
                      >
                        {course.level}
                      </Badge>
                    </div>
                  ...
                  <CardHeader className='px-2 pt-4 pb-2'>
                    <CardTitle className='flex items-start gap-2 text-[14px] leading-snug font-medium'>
                      {course.title}
                    </CardTitle>
                  </CardHeader>
                  ...
                  <CardContent className='space-y-3 px-2 pb-4'>
                    <div className='flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground'>
                      <span className='flex items-center gap-1'>
                        <Layers3 className='size-3' /> {course.parts}{' '}
                        {t('coursesLessons')}
                      </span>
                      <span className='flex items-center gap-1'>
        ...
        <section id='projects' className='space-y-6'>
          ...
          <div className='grid gap-4 sm:grid-cols-2 md:grid-cols-3'>
            {projects.map((project) => (
              <Link
                to='/project/$id'
                params={{ id: project.id ?? project.title }}
                key={project.title}
              >
                <Card
                  key={project.title}
                  className='group relative gap-3 overflow-hidden border p-0 backdrop-blur transition-all duration-300 hover:shadow-md dark:bg-neutral-800/40'
                >
                  {/* 🔸 Image */}
                  <div className='relative overflow-hidden'>
                    <img
                    ...
                    {/* Type badge */}
                    <span className='absolute top-3 right-3 rounded-md border border-white/20 bg-black/40 px-2 py-0.5 text-[11px] font-medium text-white backdrop-blur'>
                      {project.type}
                    </span>

                    {/* Title */}
                  </div>
                  <p className='px-4 py-0 text-sm font-semibold dark:text-white'>
                    {project.title}
                  </p>
        ...
        <div className='mx-auto max-w-6xl px-4 py-12 md:px-6'>
          <div className='grid gap-8 md:grid-cols-4'>
            ...
            {[
              {
                title: 'Platform',
                items: ['Courses', 'Projects', 'Code Sources'],
              },
              {
                title: 'Support',
                items: ['Help Center', 'Documentation', 'Community'],
              },
          ...
          <div className='mt-10 flex flex-col items-start justify-between gap-3 border-t pt-6 text-xs text-muted-foreground md:flex-row md:items-center'>
            <span>
              © {new Date().getFullYear()} Sammi. {t('footerRights')}
            </span>
            <a
              href='https://github.com'
              target='_blank'
